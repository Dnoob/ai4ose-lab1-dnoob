...
//! ## 与 Lab2 的区别
//!
//! Lab2 的批处理系统中，用户上下文直接在 `rust_main` 的局部变量中管理。
...
impl TaskControlBlock {
    ...
    pub fn init(&mut self, entry: usize) {
        ...
        // TODO 1: 设置用户栈指针
        //
        // 每个任务有自己独立的栈（self.stack 数组）。
        // 任务开始执行时，sp 寄存器必须指向栈顶。
        // 栈从高地址向低地址增长，所以"栈顶"是数组末尾的地址。
        //
        // 你需要计算栈顶地址并设置到上下文的 sp 中：
        //   栈顶 = 数组起始地址 + 数组总字节数
        //
        // 可用的方法：
        //   self.stack.as_ptr() as usize    → 数组起始地址
        //   core::mem::size_of_val(&self.stack)  → 数组总字节数
        //   *self.ctx.sp_mut() = 值          → 设置 sp 寄存器
        //
        // 你的代码写在这里（1行）：
