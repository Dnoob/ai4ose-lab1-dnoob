//! # Lab3：多道程序与分时多任务
//!
//! 本章在 Lab2 "批处理系统"的基础上，实现了一个**多道程序操作系统**，
//! 支持多个用户程序并发执行，并通过时钟中断实现抢占式调度。
//!
//! ## 需要完成的 TODO
//!
//! - TODO 2（本文件）：处理时钟中断 —— 关闭闹钟，返回 false
//! - TODO 3（本文件）：处理 yield 系统调用 —— 返回 false
//! - TODO 1 在 task.rs 中
...
///
/// 与 Lab2 的串行批处理不同，本章的多道程序系统支持：
/// - 多个任务同时驻留在内存中，每个任务拥有独立的 TCB 和用户栈
/// - 任务之间通过时间片轮转切换（抢占式调度）
/// - 任务可以主动让出 CPU（协作式调度，通过 yield）
extern "C" fn rust_main() -> ! {
    ...
    // 第三步：初始化系统调用处理
    // 比 Lab2 多了 scheduling（yield）、clock（获取时间）和 trace（追踪）
        ...
        if !tcb.finish {
            loop {
                ...
                let finish = match scause::read().cause() {
                    ...
                    // TODO 2: 处理时钟中断
                    //
                    // 时钟中断说明当前任务的时间片用完了，需要切换到下一个任务。
                    // 你需要做两件事：
                    //   1. 关闭闹钟：调用 tg_sbi::set_timer(u64::MAX)
                    //      把下次中断时间设到"无穷远"，避免反复触发
                    //   2. 返回 false（表示任务没有结束，只是时间片到了）
                    //
                    // 你的代码写在这里：
                    ...
                    Trap::Exception(Exception::UserEnvCall) => {
                        ...
                        match tcb.handle_syscall() {
                            ...
                            // ==============================================================
                            // TODO 3: 处理 yield 系统调用
                            //
                            // 用户程序主动让出 CPU，切换到下一个任务。
                            // 任务没有结束，以后还要回来继续跑。
                            //
                            // 你需要返回 false（表示任务没有结束）。
                            //
                            // 你的代码写在这里：
                            Event::Yield => {

                                todo!("TODO 3: 处理 yield")
                            }
                            // ==============================================================
